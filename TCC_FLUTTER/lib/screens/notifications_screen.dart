...
class _NotificationsScreenState extends State<NotificationsScreen> {
        ...
        for (var item in response) {
          try {
            if (item is List && item.length >= 5) {
              ...
              _notifications.add({
                'id': agendamento['id']?.toString() ?? '0',
                'clientName': agendamento['clienteNome'] ?? 'Cliente',
                'subtitle': 'Solicitação de agendamento',
                'time': _formatTime(agendamento['dataAgendamento']),
                'date': _formatFullDate(agendamento['dataAgendamento']),
                'serviceName': agendamento['servicoNome'] ?? 'Serviço',
  ...
  List<Map<String, dynamic>> _getFilteredNotifications() {
    if (_selectedTab == 'Concluídos') {
      return _notifications.where((n) => n['status'].toString().toLowerCase() != 'pendente').toList();
    } else {
      return _notifications.where((n) => n['status'].toString().toLowerCase() == 'pendente').toList();
    }
  }
